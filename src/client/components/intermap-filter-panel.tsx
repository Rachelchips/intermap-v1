...
export function IntermapFilterPanel({
  ...
  useEffect(() => {
    ...
    [...locationCats, ...eventCats].forEach((cat) => { next[`${cat.id}`] = true; });
    setExpanded(next);
  }, [locationCats.length, eventCats.length]);

  useEffect(() => {
    if (!eventFilterEnabled && activeTab === "events") {
      setActiveTab("locations");
    }
  }, [activeTab, eventFilterEnabled]);
  ...
  const toggleAll = useCallback((cat: TagCategory) => {
    ...
    updateCurrentFilter({ ...currentFilter, [cat.id]: allOn ? new Set<string>() : new Set(allIds) });
  }, [currentFilter, updateCurrentFilter]);
  ...
  return (
    <>
      ...
      <div style={{
        position: "absolute", top: 0, left: 0, height: "100%", width: 280, zIndex: 45,
        ...
        <div style={{
          padding: "12px 16px 10px", borderBottom: `1px solid ${borderColor}44`,
          ...
          <button onClick={() => setOpen(false)}
            style={{ background: "none", border: "none", cursor: "pointer", color: themeMuted, padding: 2, display: "flex", alignItems: "center" }}>
            <X size={14} />
          </button>
        </div>

        <div style={{ padding: "10px 16px 8px", borderBottom: `1px solid ${borderColor}22`, display: "flex", gap: 6, flexShrink: 0 }}>
          <button
          ...
          <button
            onClick={() => eventFilterEnabled && setActiveTab("events")}
            disabled={!eventFilterEnabled}
            style={{
              ...tabStyle(activeTab === "events", themeColor, themeMuted, borderColor),
              opacity: eventFilterEnabled ? 1 : 0.45,
              cursor: eventFilterEnabled ? "pointer" : "not-allowed",
            }}
          >
            事件
          </button>
        ...
        <div style={{ padding: "8px 16px", borderBottom: `1px solid ${borderColor}22`, flexShrink: 0 }}>
          <button onClick={resetCurrent}
            style={{
              fontSize: 11, color: themeMuted, background: "none",
              border: `1px solid ${borderColor}66`, borderRadius: 4,
              padding: "3px 10px", cursor: "pointer", width: "100%",
            }}>
            重置当前筛选
          </button>
          {!eventFilterEnabled && (
            <div style={{ marginTop: 8, fontSize: 11, color: themeMuted, opacity: 0.7 }}>
              事件收缩显示下，只有地图上同时显示 2 个及以上事件时才能筛选事件。
            </div>
          )}
        </div>
                    ...
                    return (
                      <div key={val.id}
                        ...
                        style={{
                          ...
                          padding: "4px 16px 4px 28px", cursor: "pointer",
                          opacity: checked ? 1 : 0.42, transition: "opacity 0.15s",
                        }}>
                        <div style={{
